#define SAMPLES 5
#define accelerationThreshold 100  
...
void loop() 
{
  ...
  int accelDifference = abs( accel.xRaw-accel.xAxis ) + abs( accel.yRaw-accel.yAxis) + abs( accel.zRaw-accel.zAxis);
    ...
    if (accelDifference > accelerationThreshold) 
    {
      ...
      if (accelDifference > 200 && yAng > 0.3) 
      {
      ...
      } else if (accelDifference > 300 && yAng < -0.7) 
      {
        //Serial.println(accelDifference);
        //Serial.println(yAng);
        String stringToPrint = String();
  ...
  delay(200); //200ms -- might have to change to delay if pixels use millis()
